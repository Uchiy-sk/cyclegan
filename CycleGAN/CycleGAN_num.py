...
# Check points
checkpoint_path = "./checkpoints/num_0.02"
...
def generate_images(model, test_input, num):
    ...
    filename = f"C:\\Users\\uchiyama\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\holo_epoch_{num+1}.jpg"
    # filename = f"C:\\Users\\Uchiy\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\holo_epoch_{num+1}.jpg"

    test_input = np.stack(test_input).reshape(1, 256, 256, 3)
    prediction = np.stack(prediction).reshape(1, 256, 256, 3)
...
for epoch in range(EPOCHS):
    ...
    # culclate FID
    real_g = np.stack(holo_train_data)
    real_f = np.stack(mnist_train_data)
    print("real_g -> ", real_g.shape)
    print("real_f -> ", real_f.shape)

    fake_g = generator_g(sample_mnist)
    fake_f = generator_f(sample_holo)
    ...
    fid_mnist = get_fid(real_g, fake_g)
    fid_holo = get_fid(real_f, fake_f)
    print("FID_g : ", fid_mnist)
    print("FID_f : ", fid_holo)
    FID_mnist.append(fid_mnist)
    FID_holo.append(fid_holo)

    #culclate SSIM, PSNR
    ssim_g = tf.image.ssim(real_g[0], fake_g[0], max_val=1.0)
    psnr_g = tf.image.psnr(real_g[0], fake_g[0], max_val=1.0)
    print("ssim_g = ", ssim_g.numpy())
    print("psnr_g = ", psnr_g.numpy())

    ssim_f = tf.image.ssim(real_f[0], fake_f[0], max_val=1.0)
    psnr_f = tf.image.psnr(real_f[0], fake_f[0], max_val=1.0)
    print("ssim_2 = ", ssim_f.numpy())
    print("psnr_2 = ", psnr_f.numpy())
...
plt.savefig("C:\\Users\\uchiyama\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\mnist_gen_loss.png")
# plt.savefig("C:\\Users\\Uchiy\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\mnist_gen_loss.png")
...
plt.savefig("C:\\Users\\uchiyama\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\mnist_disc_loss.png")
# plt.savefig("C:\\Users\\Uchiy\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\mnist_disc_loss.png")

plt.figure(figsize=(8, 6))

plt.xlabel('epoch')
plt.ylabel('FID')
plt.plot(range(len(FID_mnist)), FID_mnist, marker='o', label='g_loss')
plt.plot(range(len(FID_holo)), FID_holo, marker='^', label='f_loss')
plt.legend(loc='best', fontsize=10)
plt.grid()

plt.savefig("C:\\Users\\uchiyama\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\cyclegan_fid.png")
# plt.savefig("C:\\Users\\Uchiy\\OneDrive - 千葉大学\\ドキュメント\\Python\\research\\picture\\cyclegan_holo\\z=0.02(1000)\\λ=10\\cyclegan_fid.png")
