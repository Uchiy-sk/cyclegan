...
def asm(img, z, lam):
    ...
    # 255段調化
    amp = np.zeros_like(ift.shape, dtype=float)
    amp = 255 * (ift.real - min) / (max - min)
    # amp = ift.real
    # print("amp->\n", amp)

    # トリミング
    trim = np.zeros((int(amp.shape[0]/2), int(amp.shape[1]/2)), dtype="int64")
    trim = amp[int(amp.shape[0]/4):int(amp.shape[0]*3/4),
               int(amp.shape[1]/4):int(amp.shape[0]*3/4)]
